...
class UIManager(IUIManagerInterface):
    ...
    def get_any_tool_tip_on_screen(self) -> bool:
        """
        Return True when at least one manager-owned tool tip is currently visible on screen.

        :return: True if any tool tip is alive, visible, and intersects the root window.
        """
        window_rect = pygame.Rect(0, 0, 0, 0)
        root_container = self.get_root_container()
        if root_container is not None:
            window_rect = root_container.get_rect()

        return any(
            isinstance(sprite, UITooltip)
            and sprite.alive()
            and sprite.visible
            and sprite.rect.colliderect(window_rect)
            for sprite in self.ui_group.sprites()
        )
