...
class UITooltip(UIElement, IUITooltipInterface):
    ...
    def find_valid_position(self, position: Coordinate) -> bool:
        ...
        hover_x = self.hover_distance_from_target[0]
        hover_y = self.hover_distance_from_target[1]

        # Prefer below and above placement first and clamp horizontally so vertical placement
        # does not fail near screen edges when there is otherwise enough room.
        below_rect = self._get_horizontally_clamped_rect(window_rect, hover_x)
        below_rect.left = target_x
        below_rect.top = int(target_y + hover_y)
        below_rect = self._get_horizontally_clamped_rect(window_rect, hover_x, below_rect)
        if window_rect.contains(below_rect):
            self.rect = below_rect
            return self._copy_rect_to_rel_and_set_text_pos()

        above_rect = self._get_horizontally_clamped_rect(window_rect, hover_x)
        above_rect.left = target_x
        above_rect.bottom = int(target_y - hover_y)
        above_rect = self._get_horizontally_clamped_rect(window_rect, hover_x, above_rect)
        if window_rect.contains(above_rect):
            self.rect = above_rect
            return self._copy_rect_to_rel_and_set_text_pos()

        # If vertical placement fails, try right then left with target centered vertically.
        # Keep at least one pixel horizontal gap so the tooltip does not sit directly under
        # the cursor when hover_x is 0.
        side_gap = max(hover_x, 1)

        right_rect = self.rect.copy()
        right_rect.left = int(target_x + side_gap)
        right_rect.centery = target_y
        if window_rect.contains(right_rect):
            self.rect = right_rect
            return self._copy_rect_to_rel_and_set_text_pos()

        left_rect = self.rect.copy()
        left_rect.right = int(target_x - side_gap)
        left_rect.centery = target_y
        if window_rect.contains(left_rect):
            self.rect = left_rect
            return self._copy_rect_to_rel_and_set_text_pos()

        # Final placement attempts keep the tooltip on the requested side while
        # vertically centering it on screen.
        centered_right_rect = self.rect.copy()
        centered_right_rect.left = int(target_x + side_gap)
        centered_right_rect.centery = window_rect.centery
        if window_rect.contains(centered_right_rect):
            self.rect = centered_right_rect
            return self._copy_rect_to_rel_and_set_text_pos()

        centered_left_rect = self.rect.copy()
        centered_left_rect.right = int(target_x - side_gap)
        centered_left_rect.centery = window_rect.centery
        if window_rect.contains(centered_left_rect):
            self.rect = centered_left_rect
            return self._copy_rect_to_rel_and_set_text_pos()

        self.rect = centered_left_rect
        return self._copy_rect_to_rel_and_warn("Unable to fit tool tip on screen")

    def _get_horizontally_clamped_rect(
        self,
        window_rect: pygame.Rect,
        hover_x: int,
        rect_to_clamp: Optional[pygame.Rect] = None,
    ) -> pygame.Rect:
        clamped_rect = self.rect.copy() if rect_to_clamp is None else rect_to_clamp.copy()
        if clamped_rect.right > window_rect.right:
            clamped_rect.right = window_rect.right - hover_x
        if clamped_rect.left < window_rect.left:
            clamped_rect.left = window_rect.left + hover_x
        return clamped_rect
