...
def report_generate_heatmap(data):
    """
    Visualize data generated by create_report() as a heatmap
    """
    ...
    row_height = 0.4  # Adjust this to control vertical spacing
    col_width = 0.8   # Adjust this to control horizontal spacing
    figsize = (cols * col_width, rows * row_height)
    
    plt.figure(figsize=figsize)
    sns.heatmap(heatmap_data, annot=True, fmt=".1f", cmap="coolwarm")
    plt.title("Win Rates of Characters with Different Equipment Sets", fontsize=16)
    plt.xlabel("Equipment Set", fontsize=12)
...
def report_export_pivot_table(data):
    """
    Export a pivot table from the data to a CSV file for easier analysis in Excel or similar software.
    """
    pivot_table = data.pivot(index='character_name', columns='equipment_set', values='winrate')
    pivot_table.fillna(0, inplace=True)  # Replace NaN with 0 for easier visualization in Excel
    output_file = './reports/pivot_table.csv'
    pivot_table.to_csv(output_file)
    print(f"Pivot table exported to {output_file}")
